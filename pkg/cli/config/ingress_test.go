...
func TestGetIngresses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != (tt.expectedCount == 0 && len(tt.setupIngresses) == 0) {
				// Only expect error if we have no ingresses at all
				if tt.expectedCount == 0 && len(tt.setupIngresses) > 0 {
					// This is expected - no tekton results ingresses found
				} else if err != nil {
